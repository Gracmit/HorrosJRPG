    ...
    #ifdef PARALLAX
        void ParallaxUV(inout float2 uv[_LAYER_COUNT], float3 tangentViewDir)
        {
        #define uvParallax(i)                                                                                                                                       \
        uv[i] += ParallaxOffset(_Mask##i, sampler_Splat0, _DiffuseRemapOffset##i.w, _DiffuseRemapScale##i.w, uv[i], tangentViewDir, _ParallaxAffineStepsTerrain);   \
    ...
    {   
        ...
        void SampleSplatTOL(out float4 mixedAlbedo, out float3 mixedNormal, float4 noTriplanarAlbedo, float3 noTriplanarNormal, float2 uv[_LAYER_COUNT], float4 blendMask[2], float4 mask[_LAYER_COUNT])
        {
            ...
            albedo[0].rgb *= _DiffuseRemapScale0.rgb;
