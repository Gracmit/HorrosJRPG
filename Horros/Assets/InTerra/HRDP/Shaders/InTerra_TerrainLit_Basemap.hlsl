...
SAMPLER(sampler_TerrainColorTintTexture);

float _TriplanarSharpness, _TriplanarOneToAllSteep, _TerrainColorTintStrenght;
float4 _MainTex_ST;
...
void InTerraTerrainLitShade(float2 uv, inout TerrainLitSurfaceData surfaceData, float3 positionWS, float3 normalWS, float3 tangentViewDir)
{
    ...
    float4 triplanarTex = SAMPLE_TEXTURE2D(_TriplanarTex, sampler_TriplanarTex, uv);
        ...
        float3 tint = SAMPLE_TEXTURE2D(_TerrainColorTintTexture, sampler_TerrainColorTintTexture, uv).rgb;
