...
void TriplanarBase(in out half4 baseMap, half4 front, half4 side, float3 weights, float2 splat, fixed firstToAllSteep)
{
    ...
    float GetParallaxHeight(sampler2D mask, float2 uv, float2 offset)
    {
        return tex2Dlod(mask, float4(uv + offset, 0, 0) ).b;
    }

    //this function is based on Parallax Occlusion Mapping from Unity Shader Graph URP/HDRP
    float2 ParallaxOffset(sampler2D mask, int numSteps, float amplitude, float2 uv, float3 tangentViewDir, float affineSteps)
    {
        float2 offset = 0;
        ...
        if (numSteps > 0)
        {
            ...
            float prevHeight = GetParallaxHeight(mask, uv, texOffsetCurrent);
            texOffsetCurrent += texOffsetPerStep;
            float currHeight = GetParallaxHeight(mask, uv, texOffsetCurrent);
            ...
            for (int stepIndex = 0; stepIndex < numSteps; ++stepIndex)
            {
                ...
                currHeight = GetParallaxHeight(mask, uv, texOffsetCurrent);
            }
            ...
            else
            {
                ...
                for (int i = 0; i < affineSteps; ++i)
                {
                    ...
                    currHeight = GetParallaxHeight(mask, uv, offset);
...
//=======================================================================================
    ...
    half Metallic(half4 splat_control)
    {
        ...
        {
            uv += ParallaxOffset(_Mask0, _DiffuseRemapOffset0.w, _DiffuseRemapScale0.w, uv, tangentViewDir, _ParallaxAffineStepsTerrain);
        }
    #endif 
#endif 
...
//=======================================================================================
    ...
    void SampleSplatTOL(out half4 splat[2], half4 noTriplanarSplat[2], float2 uv, half defaultAlpha, float4 splat_control, half4 mask)
    {
        ...
        {
            uv[0] += ParallaxOffset(_Mask0, _DiffuseRemapOffset0.w, _DiffuseRemapScale0.w, uv[0], tangentViewDir, _ParallaxAffineStepsTerrain);
            uv[1] += ParallaxOffset(_Mask1, _DiffuseRemapOffset1.w, _DiffuseRemapScale1.w, uv[1], tangentViewDir, _ParallaxAffineStepsTerrain);
        }
    #endif 
#endif
...
//=======================================================================================
    ...
    void DistantUV(out float2 distantUV[4], float2 uvSplat[4])
    {
        ...
        void ParallaxUV(inout float2 uv[4], float3 tangentViewDir)
        {
            uv[0] += ParallaxOffset(_Mask0, _DiffuseRemapOffset0.w, _DiffuseRemapScale0.w, uv[0], tangentViewDir, _ParallaxAffineStepsTerrain);
            uv[1] += ParallaxOffset(_Mask1, _DiffuseRemapOffset1.w, _DiffuseRemapScale1.w, uv[1], tangentViewDir, _ParallaxAffineStepsTerrain);
            uv[2] += ParallaxOffset(_Mask2, _DiffuseRemapOffset2.w, _DiffuseRemapScale2.w, uv[2], tangentViewDir, _ParallaxAffineStepsTerrain);
            uv[3] += ParallaxOffset(_Mask3, _DiffuseRemapOffset3.w, _DiffuseRemapScale3.w, uv[3], tangentViewDir, _ParallaxAffineStepsTerrain);
        }
    #endif 
