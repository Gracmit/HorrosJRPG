...
void SplatmapVert(inout appdata_full v, out Input data)
{
    ...
    //================================   OBJECTS INTEGRATION  ==============================
        ...
        data.mainTC_tWeightY_hOffset = float4(v.texcoord.xy, tWeights.y, heightOffset);
            ...
            half3 tangentY = normalize(cross(wNormal, half3(0, 0, axisSign.y)));
            half3 bitangentY = normalize(cross(tangentY, wNormal)) * axisSign.y;
...
#endif
{
    ...
    mixedDiffuse = 1;
        ...
        float2 mainUV = TRANSFORM_TEX(IN.mainTC_tWeightY_hOffset.xy, _MainTex);
            ...
            mainParallaxOffset = ParallaxOffset(_MainMask, _ParallaxSteps, _ParallaxHeight, IN.mainTC_tWeightY_hOffset.xy, normalize(IN.tangentViewDirObject), _ParallaxAffineSteps);
    ...
    //--------------------------- SPLAT MAP CONTROL ----------------------------------
        ...
        float2 terrainUV = (IN.worldPos.xz - _TerrainPosition.xz) * (1 / _TerrainSize.xz);
            ...
            half4 tint = tex2D(_TerrainColorTintTexture, terrainUV);
        #endif
    #else
        float2 splatMapUV = (IN.tc.xy * (_Control_TexelSize.zw - 1.0f) + 0.5f) * _Control_TexelSize.xy;
        #if defined(_TERRAIN_TINT_TEXTURE) || !defined(DIFFUSE)
            half4 tint = tex2D(_TerrainColorTintTexture, IN.tc.xy);
        #endif
    #endif 
    ...
    //-------------------- PARALLAX OFFSET -------------------------                   
    #ifdef _TERRAIN_PARALLAX
        ParallaxUV(uvSplat, normalize(IN.tangentViewDir));
    #endif
    ...
    //=======================================================================================
    #ifdef INTERRA_OBJECT	
        float steepWeights = _SteepIntersection == 1 ? saturate(IN.wOrigNormal.y + _Steepness) : 1;
