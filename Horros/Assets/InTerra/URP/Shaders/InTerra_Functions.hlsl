...
#if defined (PARALLAX)
    float GetParallaxHeight(texture2D maskT, sampler maskS, float2 uv, float2 offset)
    {
        return SAMPLE_TEXTURE2D_LOD(maskT, maskS, float2(uv + offset), 0).b;    
    }

    //this function is based on Parallax Occlusion Mapping from Shader Graph URP
    float2 ParallaxOffset(texture2D maskT, sampler maskS, int numSteps, float amplitude, float2 uv, float3 tangentViewDir, float affineSteps)
    {    
        float2 offset = 0;
        ...
        if (numSteps > 0)
        {
            ...
            float prevHeight = GetParallaxHeight(maskT, maskS, uv, texOffsetCurrent);
            texOffsetCurrent += texOffsetPerStep;
            float currHeight = GetParallaxHeight(maskT, maskS, uv, texOffsetCurrent);
            ...
            for (int stepIndex = 0; stepIndex < numSteps; ++stepIndex)
            {
                ...
                currHeight = GetParallaxHeight(maskT, maskS, uv, texOffsetCurrent);
            }
               ...
               for (int i = 0; i < affineSteps; ++i)
                {
                    ...
                    currHeight = GetParallaxHeight(maskT, maskS, uv, offset);
...
//=========================================================================================
    ...
    #ifdef PARALLAX
        void ParallaxUV(inout float2 uv, float3 tangentViewDir)
        {
            uv += ParallaxOffset(_Mask0, sampler_Mask0, _DiffuseRemapOffset0.w, _DiffuseRemapScale0.w, uv, tangentViewDir, _ParallaxAffineStepsTerrain);
        }
    #endif 
...
#ifdef PARALLAX
    void ParallaxUV(inout float2 uv[2], float3 tangentViewDir)
    {
        uv[0] += ParallaxOffset(_Mask0, sampler_Mask0, _DiffuseRemapOffset0.w, _DiffuseRemapScale0.w, uv[0], tangentViewDir, _ParallaxAffineStepsTerrain);
        uv[1] += ParallaxOffset(_Mask1, sampler_Mask0, _DiffuseRemapOffset1.w, _DiffuseRemapScale1.w, uv[1], tangentViewDir, _ParallaxAffineStepsTerrain);
    }
#endif 
...
//=========================================================================================
    ...
    #ifdef TERRAIN_MASK
        void SampleMask(out half4 masks[4], half4 hasMask, float2 uv[4]) //void SampleMask(out half4 mask[4], float2 uv[4])
        {
            masks[0] = 0.5h;
            masks[1] = 0.5h;
            masks[2] = 0.5h;
    ...
    #ifdef PARALLAX
        void ParallaxUV(inout float2 uv[4], float3 tangentViewDir)
        {
            uv[0] += ParallaxOffset(_Mask0, sampler_Mask0, _DiffuseRemapOffset0.w, _DiffuseRemapScale0.w, uv[0], tangentViewDir, _ParallaxAffineStepsTerrain);
            uv[1] += ParallaxOffset(_Mask1, sampler_Mask0, _DiffuseRemapOffset1.w, _DiffuseRemapScale1.w, uv[1], tangentViewDir, _ParallaxAffineStepsTerrain);
            uv[2] += ParallaxOffset(_Mask2, sampler_Mask0, _DiffuseRemapOffset2.w, _DiffuseRemapScale2.w, uv[2], tangentViewDir, _ParallaxAffineStepsTerrain);
            uv[3] += ParallaxOffset(_Mask3, sampler_Mask0, _DiffuseRemapOffset3.w, _DiffuseRemapScale3.w, uv[3], tangentViewDir, _ParallaxAffineStepsTerrain);
        }
    #endif 
