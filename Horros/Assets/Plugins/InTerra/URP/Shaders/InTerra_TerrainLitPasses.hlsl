...
#endif
{
    ...
    half4 metallic_AO_splat01 = SAMPLE_TEXTURE2D(_MetallicTex, sampler_MetallicTex, IN.uvMainAndLM.xy);
        ...
        if (_NumLayersCount <= 4)
        {
            ...
            float3 tint = SAMPLE_TEXTURE2D(_TerrainColorTintTexture, sampler_TerrainColorTintTexture, IN.uvMainAndLM.xy).rgb;
    ...
    float3 tangentViewDir = float3(0,0,0);
        ...
        tangentViewDir = -normalize(mul(objectToTangent, GetWorldSpaceViewDir(float4(IN.positionWS, 0))).xyz);
    #endif

    #if defined(_TERRAIN_TRIPLANAR) || defined(_TERRAIN_TRIPLANAR_ONE) || defined(PARALLAX)
        SplatmapMix(IN.uvMainAndLM, IN.uvSplat01, IN.uvSplat23, IN.normal, tangentViewDir, IN.positionWS, splatControl, weight, mixedDiffuse, smoothness, metallic, occlusion, normalTS);
    #else
