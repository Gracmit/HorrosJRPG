...
namespace InTerra
{
	...
	{	
		...
		public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
		{
			...
			if (targetMat.shader.name == InTerra_Data.HDRPObjectShaderName)
			{
			...
			{ 
				using (new GUILayout.VerticalScope(EditorStyles.helpBox))
				{
					...
					if (maskTex.textureValue == null)
					{
						...
						if (targetMat.shader.name == InTerra_Data.URPObjectShaderName || targetMat.shader.name == InTerra_Data.HDRPObjectShaderName)
						{
				...
				using (new GUILayout.VerticalScope(EditorStyles.helpBox))
				{
					...
					if (parallax)
					{
						using (new GUILayout.VerticalScope(EditorStyles.helpBox))
						{
							...
							using (new GUILayout.HorizontalScope())
							{
								parallaxHeight = EditorGUILayout.FloatField(LabelAndTooltip("Height:", "The value of the height illusion."), parallaxHeight, GUILayout.MinWidth(60));
			...
			{			
				...
				{
					EditorGUILayout.HelpBox("There is no Terrain or this Material is not assigned to any Object in this Scene.", MessageType.Info);
				}
				else
				{
